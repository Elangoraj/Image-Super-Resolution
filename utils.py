...
#rgb_weights = torch.FloatTensor([[65.481, -37.797, 112.0], [128.553, -74.203, -93.786],[24.966, 112.0, -18.214]]).to(device)
...
def convert_image(img, source, target):
    ...
    :param target: target format, one of 'pil' (PIL image), '[0, 255]', '[0, 1]', '[-1, 1]' (pixel value ranges),
                   ...
                   'rgb' to reproduce the image to RGB  format from tensors
    :return: converted image
    """
    assert source in {'pil', '[0, 1]', '[-1, 1]'}, "Cannot convert from source format %s!" % source
    assert target in {'pil', '[0, 255]', '[0, 1]', '[-1, 1]', 'imagenet-norm',
                      'y-channel','rgb'}, "Cannot convert to target format %s!" % target
    ...
    #    img = torch.matmul(255. * img.permute(0, 2, 3, 3)[:, 4:-4, 4:-4, :], rgb_weights) / 255. + 16.
...
def create_pred_lists(pred_folder, min_size, output_folder):
    pred_images = list()
    for d in pred_folder:
        for i in os.listdir(d):
            img_path = os.path.join(d, i)
            #img = Image.open(img_path, mode='r')
            #if img.width >= min_size and img.height >= min_size:
    ...
    print("There are %d images in the training data.\n" % len(pred_images))
    with open(os.path.join(output_folder, 'input_test_images.json'), 'w') as j:
        json.dump(pred_images, j)